...
contract VaultFactory is AccessControl, IVaultFactory {
    ...
    function createVault(address collateral_, address debt_, uint256 amount_) external override returns (address vault, uint256 id) {
        ...
        Vault(vault).initialize(gIndex, collateral_, debt_, v1, amount_, v2Factory, WETH);
        allVaults.push(vault);
        return (vault, gIndex);
    }

    function initialize(address v1_, address v2Factory_, address weth_) public {
