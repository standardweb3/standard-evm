...
exports.getVerificationStatus = exports.verifyContract = exports.delay = void 0;
...
async function verifyContract(url, req) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new plugins_1.NomicLabsHardhatPluginError(pluginContext_1.pluginName, `The HTTP server response is not ok. Status code: ${response.status} Response text: ${responseText}`);
        }
        const etherscanResponse = new EtherscanResponse(await response.json());
        if (!etherscanResponse.isOk()) {
            throw new plugins_1.NomicLabsHardhatPluginError(pluginContext_1.pluginName, etherscanResponse.message);
        }
        return etherscanResponse;
    }
    catch (error) {
        throw new plugins_1.NomicLabsHardhatPluginError(pluginContext_1.pluginName, `Failed to send contract verification request.
Endpoint URL: ${url}
...
async function getVerificationStatus(url, req) {
    ...
    try {
        ...
        throw new plugins_1.NomicLabsHardhatPluginError(pluginContext_1.pluginName, `Failure during etherscan status polling. The verification may still succeed but
...
Reason: ${error.message}`, error);
    ...
    if (!etherscanResponse.isOk()) {
        throw new plugins_1.NomicLabsHardhatPluginError(pluginContext_1.pluginName, `The Etherscan API responded with a failure status.
