...
contract InternationalOracleRegistry is AccessControl {
    ...
    mapping(address => mapping(string => address)) public PriceFeeds;

    constructor() {
        _setupRole(ORACLE_OPERATOR_ROLE, _msgSender());
    }

    function _getPriceOf(address asset_, string memory fiat_) internal view returns (int256) {
    ...
    function addOracle(address asset_, string memory fiat_, address aggregator_) public {
        require(
            hasRole(ORACLE_OPERATOR_ROLE, msg.sender),
            "Meter: Caller is not an Oracle Operator"
        );
        PriceFeeds[asset_][fiat_] = aggregator_;
